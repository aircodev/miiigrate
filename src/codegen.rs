...
//!
//! Type mapping follows kysely-codegen's conventions, simplified to plain
//! interfaces (no `ColumnType`/`Generated` wrappers):
//!
//! - `numeric`/`decimal`/`int8` → `string` (loss-free)
//! - `timestamp`/`timestamptz`/`date` → `Date | string` (drivers return
//!   either depending on transport; over the iii wire it is a string)
//! - `json`/`jsonb` → `Json` (emitted alias)
//! - `bytea`/`blob` → `Buffer`
...
fn pg_udt_to_ts(udt: &str, enums: &[EnumIr]) -> String {
    match udt {
        ...
        "date" | "timestamp" | "timestamptz" => "Date | string".into(),
...
pub fn emit_typescript(ir: &SchemaIr, dialect: &str) -> String {
    ...
    out.push_str(&format!(
        "/**\n * generated by miiigrate, do not edit.\n *\n * Dialect: {dialect}. One interface per table; nullable columns are\n * `| null`; numeric/int8 map to `string`, timestamps to `Date | string`.\n */\n\n"
    ));
