...
`<dir>/YYYYMMDDHHMMSS_slug.sql`, applied in file-name order. The full file name is the primary key in the tracking table `_iii_migrations` (`name`, `checksum` SHA-256 hex, `applied_at`), created automatically. Files are **forward-only**: once applied, a file must never change — `migrate::up` verifies checksums and refuses to run on drift (`CHECKSUM_MISMATCH`). Checksums are computed with line endings normalized (`\r\n` → `\n`), so a CRLF rewrite by a Windows editor or `core.autocrlf` does not change a migration's identity.
...
One `export interface` per table plus an aggregate `Database` type, `generated by miiigrate, do not edit` header. Mappings start from kysely-codegen's conventions but describe values **as they actually arrive through `database::query`** — i.e. JSON. Types JSON cannot carry (`Date`, `Buffer`) are never emitted:

- `int2`/`int4`/`float4`/`float8` → `number`; `int8`/`numeric` → `string` (loss-free)
- `timestamp`/`timestamptz`/`date` → `string` (the database worker serializes timestamps as RFC 3339 strings)
- `bytea`/`BLOB` → `string` (base64 on the wire)
- `json`/`jsonb` → emitted `Json` alias (real JSON on the wire)
